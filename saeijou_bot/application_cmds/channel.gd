...
var data = ApplicationCommand.new()\
	super.set_name("channel")\
	super.set_description("Example channel option. Set the channel where to send logs.")\
	super.add_option(ApplicationCommand.channel_option("channel", "The channel to send logs to.", {
		"required": true,
