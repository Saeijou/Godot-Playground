...
func on_interaction(main, bot: DiscordBot, interaction: DiscordInteraction, data: Dictionary) -> void:
	...
	match custom_id:
		"set":
			...
			await interaction.update(_make_message(main.interactions[msg_id])).completed
		...
		"set-language":
			yield(interaction.update({
				"content": ":white_check_mark: Your languages was updated to: `" + PackedStringArray(interaction.data."`, `".join(values)) + "`",
				"embeds": [],
				"components": []
			}), "completed")
