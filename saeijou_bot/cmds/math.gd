...
func on_ready(main, bot: DiscordBot):
	main.connect("interaction_create", Callable(self, "on_interaction"))
...
func on_message(main, bot: DiscordBot, message: Message, channel: Dictionary, args: Array) -> void:
	...
	var msg = await bot.send(message, _make_message(data)).completed
...
func _make_message(data: Dictionary) -> Dictionary:
	...
	return {
		"embeds": [
			Embed.new().set_color("#73c6f0").set_title("Math Quiz Game")                                          super.set_footer("Requested by " + data.author_tag).set_timestamp()                                               super.set_description("Correct: %s" % str(data.correct_num)),
...
func _gen_question() -> Dictionary:
	...
	var rand_inc = floor(randf_range(-20, 20))
	if rand_inc == 0:
		rand_inc = randi() % 10 + 1
