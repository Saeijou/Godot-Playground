...
func send(messageorchannelid, content, options: Dictionary = {}) -> Message:
	# channel
	var res = await _send_message_request(messageorchannelid, content, options).completed
	return res


func reply(message: Message, content, options: Dictionary = {}) -> Message:
	options.message_reference = {'message_id': message.id}
	var res = await _send_message_request(message, content, options).completed
	return res
...
func delete(message: Message):
	var res = await _send_message_request(message, '', {}, HTTPClient.METHOD_DELETE).completed
	return res
...
func get_guild_emojis(guild_id: String) -> Array:
	var res = await _send_get('/guilds/%s/emojis' % guild_id).completed
	return res


func get_guild_member(guild_id: String, member_id: String) -> Dictionary:
	var member = await _send_get('/guilds/%s/members/%s' % [guild_id, member_id]).completed
	return member


func get_user(user_id: String) -> Dictionary:
	var user = await _send_get('/users/%s' % [user_id]).completed
	return user


func create_dm_channel(user_id: String) -> Dictionary:
	var res = await _send_request('/users/@me/channels', {'recipient_id': user_id}).completed
	return res


func remove_member_role(guild_id: String, member_id: String, role_id: String):
	var res = await _send_get('/guilds/%s/members/%s/roles/%s' % [guild_id, member_id, role_id], HTTPClient.METHOD_DELETE).completed
	return res


func add_member_role(guild_id: String, member_id: String, role_id: String):
	var res = await _send_request('/guilds/%s/members/%s/roles/%s' % [guild_id, member_id, role_id], {}, HTTPClient.METHOD_PUT).completed
	return res
...
func create_reaction(messageordict, custom_emoji: String) -> int:
	...
	var status_code = await _send_get('/channels/%s/messages/%s/reactions/%s/@me' % [messageordict.channel_id, messageordict.id, custom_emoji], HTTPClient.METHOD_PUT, ['Content-Length:0']).completed
	return status_code
...
func delete_reaction(messageordict, custom_emoji: String, userid: String = '@me') -> int:
	...
	var status_code = await _send_get('/channels/%s/messages/%s/reactions/%s/%s' % [messageordict.channel_id, messageordict.id, custom_emoji, userid], HTTPClient.METHOD_DELETE, ['Content-Length:0']).completed
...
func delete_reactions(messageordict, custom_emoji = '') -> int:
	...
	if custom_emoji != '':
		custom_emoji = 'a:' + custom_emoji
		status_code = await _send_get('/channels/%s/messages/%s/reactions/%s' % [messageordict.channel_id, messageordict.id, custom_emoji], HTTPClient.METHOD_DELETE, ['Content-Length:0']).completed
	else:
		status_code = await _send_get('/channels/%s/messages/%s/reactions' % [messageordict.channel_id, messageordict.id], HTTPClient.METHOD_DELETE, ['Content-Length:0']).completed
...
func get_reactions(messageordict, custom_emoji: String):
	...
	var ret = await _send_get('/channels/%s/messages/%s/reactions/%s' % [messageordict.channel_id, messageordict.id, custom_emoji]).completed
	return ret
...
func register_command(command: ApplicationCommand, guild_id: String = '') -> ApplicationCommand:
	...
	slug += '/commands'
	var res = await _send_request(slug, command._to_dict(true)).completed
...
func register_commands(commands: Array, guild_id: String = '') -> Array:
	...
	slug += '/commands'
	var res = await _send_request(slug, commands, HTTPClient.METHOD_PUT).completed
...
func delete_command(command_id: String, guild_id: String = '') -> int:
	...
	var res = await _send_get(slug, HTTPClient.METHOD_DELETE).completed
	return res
...
func delete_commands(guild_id: String = '') -> int:
	...
	slug += '/commands'
	var res = await _send_request(slug, [], HTTPClient.METHOD_PUT).completed
	return res


func is_command_or_control_pressed(command_id: String, guild_id: String = '') -> ApplicationCommand:
	var slug = '/applications/%s' % application.id
	...
	var cmd = await _send_get(slug).completed
...
func get_commands(guild_id: String = '') -> Array:
	...
	var cmds = await _send_get(slug).completed
...
func _ready() -> void:
	...
	$HeartbeatTimer.connect('timeout', Callable(self, '_send_heartbeat'))
	...
	match event_name:
		...
		'MESSAGE_CREATE':
			...
			if typeof(coroutine) == TYPE_OBJECT:
				coroutine = await coroutine.completed
				if coroutine == null:
...
func _send_raw_request(slug: String, payload: Dictionary, method = HTTPClient.METHOD_POST):
	...
	if http_client.has_response():
		...
		if response.has('retry_after'):
			...
			response = await _send_raw_request(slug, payload, method).completed
...
func _send_request(slug: String, payload, method = HTTPClient.METHOD_POST):
	...
	if response.has('retry_after'):
		# We got ratelimited
		await get_tree().create_timer(int(response.retry_after)).timeout
		response = await _send_request(slug, payload, method).completed
...
func _get_dm_channel(channel_id: String) -> Dictionary:
	...
	var data = await _send_get('/channels/%s' % channel_id).completed
	return data
...
func _send_message_request(
	...
	if payload.has('files') and payload.files and typeof(payload.files) == TYPE_ARRAY:
		...
		if typeof(coroutine) == TYPE_OBJECT:
			res = await coroutine.completed
		else:
			res = coroutine

	else:
		res = await _send_request(slug, payload, method).completed
...
func _send_dict_wss(d: Dictionary) -> void:
	var payload = JSON.new().stringify(d)
	_client.get_peer(1).put_packet(payload.to_utf8_buffer())
...
func _parse_message(message: Dictionary):
	...
	if message.has('channel_id') and message.channel_id:
		...
		if not channel:
			...
			channel = await _get_dm_channel(message.channel_id).completed
			_clean_channel(channel)
