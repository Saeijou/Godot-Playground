...
func edit_reply(options: Dictionary):
	...
	var res = await _edit_message('@original', options).completed
	replied = true
	return res
...
func delete_reply():
	...
	return await _delete_message().completed
...
#	return res
	var res = await _edit_message(msg.id, options).completed
	return res


func delete_follow_up(msg: Message):
	var res = await _delete_message(msg.id).completed
	return res
...
func _send_request(
	...
	if _fetch_reply:
		return await fetch_reply('@original').completed
	else:
		return true
