...
func read_env():
	...
	while not file.eof_reached():
		...
		var value = "=".join(PackedStringArray(tokens))
...
func _ready() -> void:
	...
	bot.TOKEN = token
	bot.INTENTS = 4609
	bot.connect("bot_ready", Callable(self, "_on_bot_ready"))
	bot.connect("message_create", Callable(self, "_on_message_create"))
	bot.connect("interaction_create", Callable(self, "_on_interaction_create"))
	bot.login()
	_load_commands(bot)
